...
export async function runTuneCommand(options: TuneCommandOptions = {}): Promise<CrossValidationReport | null> {
    ...
    const gridOptions: GridSearchOptions = {
        gridStep,
        coarseToFine: options.coarseToFine ?? true,
        metric,
    };

    const report = validator.runStratifiedKFold(
        cases,
        signalsEngine,
        kFolds,
        gridOptions,
        (foldIdx, totalFolds, foldResult) => {
            if (!options.json) {
                ...
                console.log(
                    `   • Fold ${foldIdx}/${totalFolds} (${foldResult.valCasesCount} val cases): Train ${trainAcc} → Val ${valAcc} (Base: ${baseValAcc} | Gap: ${gap.padStart(5)}) ${weightsStr}`
                );
            }
        }
    );
    ...
    const optimalWeights: HybridScorerWeights = {
    ...
    console.log('─────────────────────────────────────────────────────────────────');
    console.log(
        `   Baseline Weights:   ${Math.round(DEFAULT_WEIGHTS.semanticWeight * 100)}% Semantic | ${Math.round(DEFAULT_WEIGHTS.lexicalWeight * 100)}% BM25 | ${Math.round(DEFAULT_WEIGHTS.exactWeight * 100)}% Exact | ${Math.round(DEFAULT_WEIGHTS.metadataWeight * 100)}% Meta (Thresh: 0.20)`
    );
    ...
    console.log(
        `   Empirical Weights:  ${Math.round(rec.semanticWeight * 100)}% Semantic | ${Math.round(rec.lexicalWeight * 100)}% BM25 | ${Math.round(rec.exactWeight * 100)}% Exact | ${Math.round(rec.metadataWeight * 100)}% Meta (Thresh: ${rec.threshold?.toFixed(2) ?? '0.20'})`
    );
