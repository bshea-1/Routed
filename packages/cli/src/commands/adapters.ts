...
export async function runAdaptersCommand(options: AdaptersCommandOptions = {}): Promise<void> {
    ...
    switch (action) {
        ...
        case 'install': {
            if (options.host) {
                const res = await registry.install(options.host as HostId);
                if (options.json) {
        ...
        case 'uninstall': {
            if (options.host) {
                const res = await registry.uninstall(options.host as HostId);
                if (options.json) {
                    console.log(JSON.stringify(res, null, 2));
                    return;
                }
