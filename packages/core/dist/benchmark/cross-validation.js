...
export class CrossValidator {
    ...
    runStratifiedKFold(cases, signalsEngine, kFolds = 5, options = {}, onFoldComplete) {
        const t0 = performance.now();
        const folds = this.partitionStratifiedFolds(cases, kFolds);
        const foldResults = [];
        const oofPredictions = new Array(cases.length);
        const baselineOofPredictions = new Array(cases.length);
        ...
        const meanTrainAccuracy = Math.round((foldResults.reduce((sum, f) => sum + f.trainMetrics.top1Accuracy, 0) / kFolds) * 10) / 10;
        const meanValAccuracy = Math.round((foldResults.reduce((sum, f) => sum + f.valMetrics.top1Accuracy, 0) / kFolds) * 10) / 10;
        const meanGeneralizationGap = Math.round((foldResults.reduce((sum, f) => sum + f.generalizationGap, 0) / kFolds) * 10) / 10;
        const valAccuracyStdDev = Math.round(computeStandardDeviation(foldResults.map((f) => f.valMetrics.top1Accuracy)) * 10) / 10;
    ...
    partitionStratifiedFolds(cases, k) {
