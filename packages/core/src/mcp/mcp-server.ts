...
import { SkillMetadata, ScoredSkill } from '../types.js';
...
export class McpServer {
    ...
    public async handleMessage(request: Record<string, any>): Promise<Record<string, any> | null> {
    ...
    private async executeTool(name: string, args: Record<string, any>): Promise<string> {
        switch (name) {
            case 'route_skill': {
                ...
                if (hostFilter) {
                    matches = matches.filter((m: ScoredSkill) => m.skill.sourceHost === hostFilter);
                }
                ...
                const outputMatches = matches.map((m: ScoredSkill) => {
            ...
            case 'get_skill': {
                ...
                const skill = this.db.getSkillById(id) || this.db.getAllSkills().find((s: SkillMetadata) => s.name.toLowerCase() === id.toLowerCase());
                if (!skill) {
            ...
            case 'list_skills': {
                ...
                if (args.host) {
                    skills = skills.filter((s: SkillMetadata) => s.sourceHost === args.host);
                }
                if (args.filter) {
                    const q = String(args.filter).toLowerCase();
                    skills = skills.filter((s: SkillMetadata) =>
                ...
                return JSON.stringify({
                    total: skills.length,
                    skills: skills.map((s: SkillMetadata) => ({
                        id: s.id,
                        name: s.name,
                        description: s.description,
                        host: s.sourceHost,
