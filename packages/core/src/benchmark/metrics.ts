...
export function evaluatePredictions(
    cases: BenchmarkCase[],
    predictions: Array<{ actual: string[]; isNoSkill: boolean; latencyMs?: number }>
): { metrics: EvaluationMetrics; details: QueryEvaluationResult[] } {
    const details: QueryEvaluationResult[] = [];
...
export function computeMetricScore(metrics: EvaluationMetrics, metric: 'composite' | 'top1' | 'top3' | 'mrr' | 'f1' = 'composite'): number {
    switch (metric) {
        case 'top1':
