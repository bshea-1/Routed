...
function isSkillNegated(skill: SkillMetadata, negatedTokens: string[]): boolean {
    if (negatedTokens.length === 0) return false;
...
function getFrameworkPenalty(skill: SkillMetadata, queryLower: string): number {
    ...
    public async route(query: string, options: RouteOptions = {}): Promise<RouteResult> {
        ...
        for (const clause of clauses) {
            for (const skill of this.skills) {
                ...
                if (isSkillNegated(skill, negatedTokens)) continue;
        ...
        const getCandidateFloor = (cand: ScoredSkill): number => {
