...
export class AntigravityAdapter implements HostAdapter {
    public id = 'antigravity' as const;
    public name = 'Antigravity';
    private getAdapterDir(): string {
        return path.join(os.homedir(), '.gemini', 'config', 'skills', 'route');
    }
    ...
    public async installAdapter(): Promise<AdapterInstallResult> {
        ...
        try {
            ...
            return {
                ...
                success: true,
                adapterPath: skillPath,
                message: 'Successfully installed /route adapter for Antigravity.',
            };
        }
        catch (err) {
            return {
                ...
                success: false,
                adapterPath: skillPath,
                message: `Failed to install Antigravity adapter: ${err instanceof Error ? err.message : String(err)}`,
            };
        }
    }
    public async uninstallAdapter(): Promise<AdapterUninstallResult> {
        ...
        try {
            ...
            return {
                ...
                success: true,
                adapterPath: skillPath,
                message: 'Successfully removed /route adapter from Antigravity. All user skills remain untouched.',
            };
        }
        catch (err) {
            return {
                ...
                success: false,
                adapterPath: skillPath,
                message: `Failed to uninstall Antigravity adapter: ${err instanceof Error ? err.message : String(err)}`,
            };
        }
    }
